"""Keyword rules driving bucket scoring and discipline inference.

KEYWORD_RULES is the main lever. Format: ``{bucket: [(regex, weight), ...]}``.
Weights are 1-5; the highest single weight that fired in the winning bucket
sets the confidence label (5 -> high, 3-4 -> medium, 1-2 -> low).

Regexes are matched case-insensitively against ``title + " " + filename`` with
underscores replaced by spaces (so PIPING_AND_INSTRUMENT.pdf hits
"piping (and|&) instrument").

CRS is filename-pinned (see CRS_PATTERN) and bypasses scoring; its rule list
is intentionally empty. Documents is the fallback bucket and only fires its
keywords explicitly.

Typo tolerance policy: target real typos seen in the data with relaxed regex
(e.g. ``arra?n?g(e)?ment`` for ARRANGMENT/ARRAGEMENT, ``requ[is]+ition`` for
REQUSITION). Do NOT add generic fuzzy matching - false positives are
expensive and hard to test. When a new typo surfaces in
``output/undefined_for_review.csv``, relax the relevant pattern and pin it
...
KEYWORD_RULES: dict = {
    ...
    "CRS": [],  # filename-pinned; see CRS_PATTERN
    "Documents": [
