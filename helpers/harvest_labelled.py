...
Run from the project root::
    ...
    python helpers/harvest_labelled.py
"""
...
NUMERIC_CODE_TO_BUCKET: dict = {
    ...
    33: "Drawings",
    34: "Datasheets",
    # Specifications/Standards/Procedures - all three are Documents-side
    # in the 2-class fold (keep as Specifications subtype since that's
    # the most common element in this code's mix).
    37: "Specifications",
    39: "Specifications",
    40: "Datasheets",
    ...
    90: "Procedures_Plans",
    91: "Reports",
    # Codes 92, 94, 95, 96, 98, 99 were 'ambiguous_skipped' in the
    # 10-bucket world (couldn't pick a Documents-side bucket reliably)
    # but are unambiguously Documents in the 2-class fold. Map each to
    # the Documents subtype so they contribute to the labelled corpus.
...
# 3-letter Type code mapping reused from config.py
from config import TYPE_TO_BUCKET

# ---------------------------------------------------------------------------
# Discipline-named tabs in the Sahil/Shah indices (skip code/category sheets).
# ---------------------------------------------------------------------------
DISCIPLINE_TABS = {
...
def harvest_shah_ffd(rows: list, path: Path, source: str) -> None:
    ...
    for sheet in xl.sheet_names:
        ...
        if "DWG" in sheet.upper():
            ...
            default_bucket = None  # rely on code mapping
        else:
            continue  # skip VENDOR etc.
        for _, r in df.iterrows():
            title = str(r.get(title_col, "") or "").strip()
            if not title or title.lower() == "nan":
                continue
...
def harvest_asab_engg_dossier(rows: list, path: Path, source: str) -> None:
    """ENGG DOSSIER INDEX FINAL - many tabs; 'Description' is title.
    Tabs are by area (RDS-1..6, WS, GL, etc.) — not by bucket. Treat each row
    as Drawings (most rows are drawings) only if Drawing_No looks like a
    drawing code; otherwise skip. Conservative: skip this source for the
    high-confidence corpus (too noisy without per-row code mapping).
    """
    # Skipped on purpose - keeps the corpus clean.
    return
...
def main() -> None:
    ...
    for f in sahil_dir.glob("*"):
        ...
        if "DRAWING  INDEX" in name or "DRAWING INDEX" in name:
            harvest_drawing_index(rows, f, f"sahil_drawing_index")
        elif "TECHNICAL DOCS  INDEX" in name or "TECHNICAL DOCS INDEX" in name:
            harvest_tech_docs_index(rows, f, f"sahil_tech_docs_index")
    ...
    for f in q_dir.glob("*"):
        ...
        # documents file: bucket undetermined -> skip for ground truth

    # 5. Asab void files (each is a drawings list)
