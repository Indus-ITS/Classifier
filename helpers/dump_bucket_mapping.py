...
from helpers.harvest_labelled import NUMERIC_CODE_TO_BUCKET

OUT_PATH = Path("output/bucket_mapping.csv")

# Source xlsx for numeric code descriptions. The Sahil drawing index has a
# 'Document_Type' sheet listing all 92 numeric codes with their descriptions.
...
# 3-letter Type code descriptions. These come from the EPC dossier ('Type'
# column) and aren't documented in a separate sheet. Best-effort meanings
# based on standard EPC document-type abbreviations - reviewer should verify.
...
def main() -> None:
    ...
    # Build raw (description, bucket, class, status) tuples from both code
    # systems, then deduplicate. Multiple codes can produce the same row
    # (especially the 9 ambiguous 'Miscellaneous' numeric codes).
    ...
    # Sort: class (Drawings, Documents, blank), then bucket, then description.
